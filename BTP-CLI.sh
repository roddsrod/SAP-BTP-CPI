...
login_output=$(./btp login --url https://cli.btp.cloud.sap --user $userid --password $passw)
...
regions_output=$(./btp list accounts/available-region)
...
if [[ "$confirm" =~ ^[Yy]$ ]]; then
	...
	subaccount_output=$(./btp create accounts/subaccount --display-name "Trial" --region "$selected_region" --subdomain "$unique_subdomain")
	...
	echo ""
	echo -e "  ${CYAN}Extracted subaccount ID: ${WHITE}${BOLD}$subaccount_id${RESET}"
	echo ""
...
max_attempts=24  # Maximum number of attempts (24 * 5 seconds = 2 minutes)
attempt=1

while [ $attempt -le $max_attempts ]; do
    echo -e "  ${CYAN}Checking subaccount state (attempt $attempt of $max_attempts)...${RESET}"
    
    # Get the current state of the subaccount
    subaccount_output=$(./btp get accounts/subaccount "$subaccount_id")
    
    # Extract the state value using grep and cut
    subaccount_state=$(echo "$subaccount_output" | grep "state:" | grep -v "state message:" | awk '{print $2}')
    
    if [ "$subaccount_state" = "OK" ]; then
        echo -e "  ${GREEN}${BOLD}✓ Subaccount is ready!${RESET}"
        break
    else
        echo -e "  ${YELLOW}Subaccount state is: $subaccount_state${RESET}"
    fi
    
    # If we've reached the maximum attempts, exit with an error
    if [ $attempt -eq $max_attempts ]; then
        echo -e "  ${RED}${BOLD}✗ Subaccount did not become ready within the timeout period.${RESET}"
        exit 1
    fi
    
    # Wait for 10 seconds before checking again
    echo -e "  ${CYAN}Waiting 5 seconds before next check...${RESET}"
    sleep 5
...
cf_creation_output=$(./btp create accounts/environment-instance --subaccount "$subaccount_id" --environment "cloudfoundry" --service "cloudfoundry" --plan "trial" --display-name "${unique_subdomain}_Trial" --parameters "{\"instance_name\": \"${unique_subdomain}_Trial\", \"org_name\": \"${global_subdomain}_${unique_subdomain}\"}")
...
if [ $? -eq 0 ]; then
    ...
    while [ $attempt -le $max_attempts ]; do
        echo -e "  ${CYAN}Checking Cloud Foundry Environment state (attempt $attempt of $max_attempts)...${RESET}"
        
        # Get the current state of the environment instance
        cf_env_output=$(./btp get accounts/environment-instance $cf_env_id --subaccount $subaccount_id)
        
        # Extract the state value using grep and cut
        cf_env_state=$(echo "$cf_env_output" | grep "state:" | grep -v "state message:" | awk '{print $2}')
        
        if [ "$cf_env_state" = "OK" ]; then
            echo -e "  ${GREEN}${BOLD}✓ Cloud Foundry Environment is ready!${RESET}"
            break
        else
            echo -e "  ${YELLOW}Cloud Foundry Environment state is: $cf_env_state${RESET}"
        fi
        
        # If we've reached the maximum attempts, exit with an error
        if [ $attempt -eq $max_attempts ]; then
            echo -e "  ${RED}${BOLD}✗ Cloud Foundry Environment did not become ready within the timeout period.${RESET}"
            exit 1
        fi
...
# Get CF API endpoint
cf_api_endpoint=$(./btp get accounts/environment-instance $cf_env_id --subaccount $subaccount_id | grep -o "api endpoint: [a-zA-Z0-9.:/\-]\+" | cut -d' ' -f3)

# Log in to Cloud Foundry
echo -e "  ${BLUE}${BOLD}=== Cloud Foundry Login ===${RESET}"
./cf login -a "$cf_api_endpoint" -u $userid -p $passw
...
echo -e "  ${CYAN}=== Adding All Required Service Plans ===${RESET}"
...
for service_plans_amount in "${service_plans_amount[@]}"; do
  ...
  ./btp assign accounts/entitlement --to-subaccount $subaccount_id --for-service $service --plan $plan --amount 1
...
for service_plans_enable in "${service_plans_enable[@]}"; do
  ...
  ./btp assign accounts/entitlement --to-subaccount $subaccount_id --for-service $service --plan $plan --enable
...
echo -e "  ${CYAN}=== Creating Services ===${RESET}"
...
./btp subscribe accounts/subaccount --subaccount $subaccount_id --to-app it-cpitrial06-prov --plan trial

echo -e ""
# Poll the subscription status until it's ready
echo -e "  ${CYAN}Waiting for Integration Suite subscription to be ready...${RESET}"
max_attempts=72  # Maximum number of attempts (72 * 5 seconds = 6 minutes)
attempt=1

echo -e ""
while [ $attempt -le $max_attempts ]; do
    echo -e "  ${CYAN}Checking subscription state (attempt $attempt of $max_attempts)...${RESET}"
    
    # Get the current state of the subscription
    subscription_output=$(./btp list accounts/subscription --subaccount $subaccount_id)
    ...
    if [ "$subscription_state" = "SUBSCRIBED" ]; then
        echo -e "  ${GREEN}${BOLD}✓ Integration Suite subscription is ready!${RESET}"
        break
    elif [ "$subscription_state" = "IN_PROCESS" ]; then
        echo -e "  ${YELLOW}Integration Suite subscription is still in process...${RESET}"
    elif [ "$subscription_state" = "FAILED" ] || [ "$subscription_state" = "NOT_SUBSCRIBED" ]; then
        echo -e "  ${RED}${BOLD}✗ Integration Suite subscription failed.${RESET}"
        exit 1
    else
        echo -e "  ${YELLOW}Integration Suite subscription state is: $subscription_state${RESET}"
    fi
    
    # If we've reached the maximum attempts, exit with an error
    if [ $attempt -eq $max_attempts ]; then
        echo -e "  ${RED}${BOLD}✗ Integration Suite subscription did not become ready within the timeout period.${RESET}"
        exit 1
    fi
...
echo "$role_collections" | while read -r role; do
  ...
  echo -e ""
  ./btp --format json list security/role-collection --subaccount "3e7d9e28-7e01-4b1d-a3ef-6b1a03f34973" | jq -r '.[].name'
...
integration_suite_url=$(./btp list accounts/subscription --subaccount "$subaccount_id" | grep "it-cpitrial06-prov" | grep -o "https://[^ ]*")
